...
def trainModel_MEAN(Y):
    ...
    residuals = (Y["consumption"]-mean).tolist()
    return mean, residuals

def doForecast_MEAN(mean, forecast_steps):
...
def evaluationScore_MEAN(residuals):
    return np.abs(residuals).sum()

def logEvaluation(customer, evaluation_score):
    f = open(output_data_path+"mean/"+customer+"_evaluation.csv", "w+")
...
def process_customer(customer, consumptions, features, forecast_steps, output_data_path):
    try:
        ...
        # STEP 3: TRAIN MODEL
        Y_train = Y.copy()
        model, residuals = trainModel_MEAN(Y_train)
        evaluation_score = evaluationScore_MEAN(residuals)
        logEvaluation(customer, evaluation_score)
        logResiduals(residuals, output_data_path, customer, Y_train)
            
        # STEP 4: PREDICTION
        Y_forecast = doForecast_MEAN(model, forecast_steps)
        Y_forecast.to_csv(output_data_path+"mean/"+customer+"_prediction.csv", index=None)
