...
def trainModel_HOURLYMEAN(Y):
    ...
    return hourly_means, residuals

def doForecast_HOURLYMEAN(hourly_means, forecast_steps):
    ...
    Y_forecast["Y"] = Y_forecast['hour'].map(hourly_means)
    return Y_forecast 

def evaluationScore_HOURLYMEAN(residuals):
    return np.abs(residuals).sum()

def logEvaluation(customer, evaluation_score):
    f = open(output_data_path+"hourlymean/"+customer+"_evaluation.csv", "w+")
...
def logResiduals(residuals, output_data_path, customer, Y_train):
    ...
    Y_train.to_csv(output_data_path+"/"+"hourlymean/"+customer+"_residuals.csv", index=None)
...
Y_forecast = doForecast_HOURLYMEAN(hourly_means, forecast_steps)
...
def process_customer(customer, consumptions, features, forecast_steps, output_data_path):
    try:
        ...
        # STEP 3: TRAIN MODEL
        Y_train = Y.copy()
        model, residuals = trainModel_HOURLYMEAN(Y_train)
        evaluation_score = evaluationScore_HOURLYMEAN(residuals)
        logEvaluation(customer, evaluation_score)
        logResiduals(residuals, output_data_path, customer, Y_train)
            
        # STEP 4: PREDICTION
        Y_forecast = doForecast_HOURLYMEAN(model, forecast_steps)
        Y_forecast.to_csv(output_data_path+"hourlymean/"+customer+"_prediction.csv", index=None)
