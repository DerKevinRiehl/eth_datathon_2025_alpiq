...
def doForecast_SARIMA(model, forecast_steps):
...
def evaluationScore_SARIMA(residuals):
    return np.abs(residuals).sum()

def logEvaluation(customer, evaluation_score):
    f = open(output_data_path+"sarima/"+customer+"_evaluation.csv", "w+")
...
def logResiduals(residuals, output_data_path, customer, Y_train):
    ...
    Y_train.to_csv(output_data_path+"/"+"sarima/"+customer+"_residuals.csv", index=None)
...
def process_customer(customer, consumptions, features, forecast_steps, output_data_path):
    try:
        ...
        # STEP 3: TRAIN MODEL
        Y_train = Y.copy()
        model, residuals = trainModel_SARIMAX(Y_train)
        evaluation_score = evaluationScore_SARIMA(residuals)
        logEvaluation(customer, evaluation_score)
        logResiduals(residuals, output_data_path, customer, Y_train)
            
        # STEP 4: PREDICTION
        Y_forecast = doForecast_SARIMA(model, forecast_steps)
        Y_forecast.to_csv(output_data_path+"sarima/"+customer+"_prediction.csv", index=None)
...
plt.subplot(3,1,1)
plt.title("DATA AND SARIMAX PREDICTION")
...
"""
# PARALLELIZED
def process_customer_wrapper(customer):
    return process_customer(customer, consumptions, features, forecast_steps, output_data_path)

# Create a pool of worker processes
with Pool(processes=max_threads) as pool:  # Adjust the number of processes as needed
    # Map the wrapper function to the list of customers
    results = pool.map(process_customer_wrapper, customer_names)
# Print results
