...
"""
# Analysis if data is nan after first time
# 
df = []
for customer in customer_names:
    print(customer)
    Y, X = getDataForCustomer(customer, consumptions, features)
    print(customer, *analyseDataConsistency(Y))
    df.append([customer, *analyseDataConsistency(Y)])
df = pd.DataFrame(df, columns=["customer", "timestamp", "later"])
"""
...
# # Determine Weekly SARIMAX (7*24h)
# P2, D2, Q2, s2 = 1, 1, 1, 7*24  # Weekly seasonality (7 days * 24 hours)
# model2 = SARIMAX(residuals,
#                 order=(p, d, q),
#                 seasonal_order=(P2, D2, Q2, s2),
#                 enforce_stationarity=False,
#                 enforce_invertibility=False)
# model_fit2 = model2.fit(disp=False)
# print(model_fit2.summary())
# residuals2 = model_fit2.resid.tolist()

# Forecast next 24 hours (example)
forecast_steps = 24*5
forecast = model_fit.forecast(steps=forecast_steps)
# forecast2 = model_fit2.forecast(steps=forecast_steps)
...
Y_forecast["Y_model1"] = forecast.tolist()
# Y_forecast["Y_model2"] = forecast2.tolist()
Y_forecast["Y"] = Y_forecast["Y_model1"] # + Y_forecast["Y_model2"]
...
plt.plot(Y_forecast["time"], Y_forecast["Y_model1"], label='Forecast', color='red')
